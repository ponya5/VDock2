<template>
  <div ref="rootEl" class="radar-container" aria-hidden="true" />
</template>

<script setup>
// Ported from reactbits.dev Radar. The renderer internals are kept verbatim
...
onMounted(() => {
  ...
  cleanup = mountRadar(rootEl.value, null, hooks, { onError: props.onError })
})
...
function mountRadar(rootEl, canvasEl, hooks, {
  speed = 1.0,
  scale = 0.5,
  ringCount = 10.0,
  spokeCount = 10.0,
