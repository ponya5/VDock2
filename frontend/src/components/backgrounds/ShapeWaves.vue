<template>
  <div
    ...
    aria-hidden="true"
  >
    <canvas ref="canvasEl" class="shape-waves__canvas" />
  </div>
</template>

<script setup>
// Ported from reactbits.dev ShapeWaves (vgpu/WebGPU). The renderer internals are
...
onMounted(() => {
  ...
  cleanup = mountShapeWaves(rootEl.value, canvasEl.value, {
    setReady: v => {
      ready.value = v
    }
  }, {
...
function mountShapeWaves(rootEl, canvasEl, hooks, {
  text = '',
