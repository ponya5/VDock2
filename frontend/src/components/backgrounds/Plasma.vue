<template>
  <div ref="rootEl" class="plasma-container" aria-hidden="true" />
</template>

<script setup>
// Ported from reactbits.dev Plasma. The renderer internals are kept verbatim
...
onMounted(() => {
  ...
  cleanup = mountPlasma(rootEl.value, null, hooks, { onError: props.onError })
})
...
function mountPlasma(rootEl, canvasEl, hooks, {
  color = '#ffffff',
  speed = 1,
  direction = 'forward',
  scale = 1,
