<template>
  <div ref="rootEl" class="lightfall-container" aria-hidden="true" />
</template>

<script setup>
// Ported from reactbits.dev Lightfall. The renderer internals are kept verbatim
...
onMounted(() => {
  ...
  cleanup = mountLightfall(rootEl.value, null, hooks, { onError: props.onError })
})
...
function mountLightfall(rootEl, canvasEl, hooks, {
  className,
  dpr,
  paused = false,
