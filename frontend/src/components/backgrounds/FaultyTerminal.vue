<template>
  <div ref="rootEl" class="faulty-terminal-container" aria-hidden="true" />
</template>

<script setup>
// Ported from reactbits.dev FaultyTerminal. The renderer internals are kept verbatim
...
onMounted(() => {
  ...
  cleanup = mountFaultyTerminal(rootEl.value, null, hooks, { onError: props.onError })
})
...
function mountFaultyTerminal(rootEl, canvasEl, hooks, {
  scale = 1,
  gridMul = [2, 1],
  digitSize = 1.5,
  timeScale = 0.3,
