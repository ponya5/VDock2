<template>
  <div ref="rootEl" class="threads-container" aria-hidden="true" />
</template>

<script setup>
// Ported from reactbits.dev Threads. The renderer internals are kept verbatim
...
onMounted(() => {
  ...
  cleanup = mountThreads(rootEl.value, null, hooks, { onError: props.onError })
})
...
function mountThreads(rootEl, canvasEl, hooks, { color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false, ...rest })  {
