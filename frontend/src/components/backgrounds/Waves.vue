<template>
  <div ref="rootEl" class="waves" aria-hidden="true">
    <canvas ref="canvasEl" class="waves-canvas" />
  </div>
</template>

<script setup>
// Ported from reactbits.dev Waves. The renderer internals are kept verbatim
...
onMounted(() => {
  ...
  cleanup = mountWaves(rootEl.value, canvasEl.value, hooks, { onError: props.onError, lineColor: '#3498db' })
})
...
function mountWaves(rootEl, canvasEl, hooks, {
  lineColor = 'black',
