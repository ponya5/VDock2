<template>
  <div
    ...
    aria-hidden="true"
  >
    <canvas ref="canvasEl" class="aero-shards__canvas" />
  </div>
</template>

<script setup>
// Ported from reactbits.dev AeroShards (vgpu/WebGPU). The renderer internals are
// kept verbatim from the upstream React component; only the React shell was
// replaced (useRef/useState/useEffect -> plain boxes + mount-time effects).
// Plain JS script: upstream code is untyped and untouched on purpose.
...
onMounted(() => {
  ...
  cleanup = mountAeroShards(rootEl.value, canvasEl.value, {
    setReady: v => {
      ready.value = v
    }
  }, {
...
function mountAeroShards(rootEl, canvasEl, hooks, {
  ...
  // React-compat shims: the catalog renders with default props only, so refs
  // become plain boxes and effects are collected then run once on mount.
...
.aero-shards[data-ready='true'] .aero-shards__canvas {
  opacity: 1;
}
