<template>
  <canvas ref="rootEl" class="shapegrid-canvas" aria-hidden="true" />
</template>

<script setup>
// Ported from reactbits.dev ShapeGrid. The renderer internals are kept verbatim
...
onMounted(() => {
  ...
  cleanup = mountShapeGrid(rootEl.value, rootEl.value, hooks, { onError: props.onError })
})
...
function mountShapeGrid(rootEl, canvasEl, hooks, {
  direction = 'right',
  speed = 1,
  borderColor = '#999',
  squareSize = 40,
  ...
  const canvasRef = { current: rootEl };
