<template>
  <div ref="rootEl" class="evil-eye-container" aria-hidden="true" />
</template>

<script setup>
// Ported from reactbits.dev EvilEye. The renderer internals are kept verbatim
...
onMounted(() => {
  ...
  cleanup = mountEvilEye(rootEl.value, null, hooks, { onError: props.onError })
})
...
function mountEvilEye(rootEl, canvasEl, hooks, {
  eyeColor = '#FF6F37',
  intensity = 1.5,
  pupilSize = 0.6,
  irisWidth = 0.25,
